...
async function parseCsvText(csv: string): Promise<ParsedTransaction[]> {
  const filePath = path.join(
    os.tmpdir(),
    `with-amt-${Date.now()}-${Math.random().toString(16).slice(2)}.csv`
  );
...
test("CSV With. Amt. / Dep. Amt. columns import debit and credit rows", async () => {
...
test("CSV With Amt / Dep Amt headers without punctuation still classify credits", async () => {
  const txs = await parseCsvText(
    [
      "Date,Narration,With Amt,Dep Amt,Balance",
      "03/05/2026,NETFLIX.COM,649.00,,124351.00",
