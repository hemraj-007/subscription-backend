...
test("dedupe preserves equal debit and credit counterparts", () => {
  const rows = [
    ["Date", "Description", "Debit", "Credit", "Balance"],
    ["03/05/2026", "Amazon", "299", "", "124351"],
    ["03/05/2026", "Amazon", "", "299", "124650"],
  ];
  const txs = parseTransactionsFromPdfContent("", rows);

  const amazon = txs.filter((t) => t.merchant === "Amazon");
  assert.equal(amazon.length, 2);
  assert.ok(amazon.some((t) => t.amount === 299 && t.type === "DEBIT"));
  assert.ok(amazon.some((t) => t.amount === 299 && t.type === "CREDIT"));
});
