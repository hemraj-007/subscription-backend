...
export async function detectUnusedSubscriptions() {
  ...
  const activeCardMerchants = new Set(
    recentTransactions.map((row) => cardMerchantKey(row.cardId, row.merchant))
  );
  ...
  for (const sub of subscriptions) {
    if (!activeCardMerchants.has(cardMerchantKey(sub.cardId, sub.merchant))) {
