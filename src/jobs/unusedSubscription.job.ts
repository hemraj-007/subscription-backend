...
export function buildLastTransactionBySubscriptionMerchant(
...
export async function detectUnusedSubscriptions() {
  ...
  const lastTxByCardMerchant =
    buildLastTransactionBySubscriptionMerchant(recentTransactions);
  ...
  for (const sub of subscriptions) {
    const lastTxDate = lastTxByCardMerchant.get(
      subscriptionActivityKey(sub.cardId, sub.merchant)
    );
