...
function activityKey(cardId: string, merchant: string) {
...
export async function detectUnusedSubscriptions() {
  ...
  const activeCardMerchants = new Set(
    recentTransactions.map((tx) => activityKey(tx.cardId, tx.merchant))
  );
  ...
  for (const sub of subscriptions) {
    if (!activeCardMerchants.has(activityKey(sub.cardId, sub.merchant))) {
